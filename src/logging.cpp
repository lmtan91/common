         ...
         const char *fmt, ... )
{
   ...
   (void) function_name_fixup( function, scratch_buf, LOG_OUTPUT_BUF_SIZE );
...
void register_logging( const char *filename, uint32_t *cats, int *level ) {
   ...
   for (std::list<file_data>::iterator i = file_data_list->begin();
      ...
      if ( strcasecmp( i->filename, filename ) > 0 ) {
         ...
         (void) file_data_list->insert( i, node );
         found = true;
         break;
      }
   }
...
void logging_set_sync_mode( const bool mode ) {
...
const char* logging_get_names() {
   ...
   log_names_buffer = static_cast<char*>( LOGGING_MALLOC( size ) );
